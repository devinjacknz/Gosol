...
func TestMarketAnalyzerIntegration(t *testing.T) {
	...
	t.Run("Market Analysis Flow", func(t *testing.T) {
		...
		assert.Greater(t, support, 100.0)
	...
	t.Run("Trading Performance Analysis", func(t *testing.T) {
		...
		assert.InDelta(t, 0.75, metrics["win_rate"], 0.1)
		assert.InDelta(t, 10.0, metrics["max_drawdown"], 5.0)
		assert.InDelta(t, 1.5, metrics["sharpe"], 0.5)
	})
